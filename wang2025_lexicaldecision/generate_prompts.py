...
# Load data
df = pd.read_csv("/Users/cyhsieh/PsychLing-101/wang2025_lexicaldecision/processed_data:/exp.csv")
...
instruction = "每次试验包括一个注视标记和一个刺激（真字或假字）。首先，屏幕中央呈现注视标记500毫秒，120毫秒后呈现刺激。如果你认为该刺激是真字，请按 j 键；如果认为是假字，请按 f 键。虽然没有反应时间限制，但请尽可能快速并尽最大准确性作答。如果你曾经见过或认识这些刺激，那么它很有可能是真字；如果你没有见过，那么它很可能不是真字。"
...
for participant in participant_list:
    ...
    for trial in trial_num:
        ...
        if not exp_trial.empty:  # Only process if trial exists for this participant
            ...
            datapoint = f'试次{trial_id}：{image}。你按下了 <<{response}>> 键。{accuracy}。反应时间为 {rt} 毫秒。\n'
