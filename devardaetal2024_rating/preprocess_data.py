"""
Preprocessing script for de Varda et al. (2024) Cloze Probability Rating Task

This script reads the raw rating data from 8 lists, extracts participant
...
def clean_rating_question(question_text: str) -> str:
    """Extract the sentence fragment from the question text."""
    q = re.sub(
        r'\s*-\s*How much would you expect to read the word\s*"\[Field-3\]"\s*as the next word of this sentence fragment:\s*\n*\s*\[Field-1\]\.\.\.', 
        "", 
        question_text
    )
    return q.strip()
...
def clean_rating_response(response: str, list_num: int) -> int:
    """Clean and standardize rating response."""
    if pd.isna(response):
        return np.nan
    
    response_str = str(response).strip()
    
    # Handle list 5 special case (encoding error: "2 - Very much" should be "5")
    if list_num == 5:
        response_str = re.sub("2 - Very much", "5", response_str)
    
    response_str = re.sub(r"-\s*Very much", "", response_str)
    response_str = re.sub(r"-\s*Not at all", "", response_str)
...
def process_list(list_num: int, item_set: pd.DataFrame) -> pd.DataFrame:
    ...
    sentence_fragments = [clean_rating_question(str(q)) for q in questions]
    ...
    for idx, row in participants_df.iterrows():
        original_participant_id = row['Q4']
        ...
        for trial_order, (col, sentence_frag) in enumerate(zip(df.columns[28:], sentence_fragments)):
            response = row[col]
            rating = clean_rating_response(response, list_num)
    ...
    for list_num in range(1, 9):
        ...
        if file_path.exists():
            ...
            for idx in range(1, len(df)):
                original_id = df.iloc[idx, df.columns.get_loc('Q4')]
                if pd.notna(original_id):
                    anon_id = generate_anon_id(original_id)
                    df.iloc[idx, df.columns.get_loc('Q4')] = anon_id
            
            csv_path = file_path.with_suffix('.csv')
            df.to_csv(csv_path, index=False)
            file_path.unlink()
            print(f"  Anonymized list{list_num}.xlsx -> list{list_num}.csv")
