# R port of guenther2023ViSpa

Native-R translation of the `guenther2023ViSpa/` pipeline. Outputs match the Python pipeline byte-for-byte.

```sh
Rscript preprocess_data.R     # original_data/ -> processed_data/exp1.csv
Rscript generate_prompts.R    # processed_data/exp1.csv -> prompts.jsonl
```

Requires `readr` and `jsonlite`:
