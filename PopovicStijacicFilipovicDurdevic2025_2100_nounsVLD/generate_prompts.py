...
df = df.sort_values(by=['participant_id', 'list', 'trial_order'])
...
# Generate individual prompts for each participant x list combination
all_prompts = []
for participant_id in participants:
    df_participant_id = df[df['participant_id'] == participant_id].copy()
    participant_id = participant_id.item()

    for list_id, df_list in df_participant_id.groupby('list'):
        rt_list = []
        ...
        df_list.loc[df_list["response"] == "1.0", "response"] = choices[0]
        df_list.loc[df_list["response"] == "3.0", "response"] = choices[1]
        ...
        for trial in sorted(df_list['trial_order'].unique()):
            df_trial = df_list[df_list['trial_order'] == trial]
        ...
        all_prompts.append({
            'text': prompt,
            'experiment': 'PopovicStijacicFilipovicDurdevic_2100_nounsVLD',
